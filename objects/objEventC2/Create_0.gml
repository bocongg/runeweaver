...
while (slot < inventory_width) {
	if (global.inventory[# slot, 0] == item.frostfirerune3 ||
		global.inventory[# slot, 0] == item.blizzardrune3 ||
		global.inventory[# slot, 0] == item.infernorune3 ||
		global.inventory[# slot, 0] == item.prismrune3){
		checkLevel3Rune = true;
		ds_list_add(inventorySpRunes, global.inventory[# slot, 0]);
	}
	slot ++;
}

// Check through all weave slots for level 3 special runes
...
while (slotWeave < weave_width) {
	if (global.weave_slot[# slotWeave, 0] == item.frostfirerune3 ||
		global.weave_slot[# slotWeave, 0] == item.blizzardrune3 ||
		global.weave_slot[# slotWeave, 0] == item.infernorune3 ||
		global.weave_slot[# slotWeave, 0] == item.prismrune3){
		checkLevel3Rune = true;
...
// Check through all attack slots for level 3 special runes
...
while (slotAttack < attack_width) {
	if (global.attack_slot[# slotAttack, 0] == item.frostfirerune3 ||
		global.attack_slot[# slotAttack, 0] == item.blizzardrune3 ||
		global.attack_slot[# slotAttack, 0] == item.infernorune3 || 
		global.attack_slot[# slotAttack, 0] == item.prismrune3){
		checkLevel3Rune = true;
