...
if (device_mouse_x_to_gui(0) > bbox_left && device_mouse_x_to_gui(0) < bbox_right && device_mouse_y_to_gui(0) > bbox_top && device_mouse_y_to_gui(0) < bbox_bottom)
{
	...
	if (is_clicked)
	{
		...
		{
			instance_destroy(objNoBackButton);
			instance_destroy(objYesSureButton);
			instance_destroy(objPauseScreen2);
			instance_create_layer(1920/2, 1080/2, "PauseScreen", objPauseScreen1);
		}
	}
} else {
	sprite_index = sprPauseButtons;
