...
// If global stage = 3, but room node 5 is cleared, room node should be locked and show UNcompleted
} else if (global.stage == 3 && ds_list_find_index(global.stagesCleared, "objRmNode5") != -1){
	draw_sprite(nodeSpr, 3, x, y);
	if (mouse_check_button_pressed(mb_left)) {
		// Sets click state to false.
		is_clicked = false;
